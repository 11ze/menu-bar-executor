...
- **文件监听**: `DispatchSource.makeFileSystemObjectSource` 监听 `[.write, .delete, .rename]`（外部就地写与原子替换都触发重载）；自身写入也会经旧 fd 绕一圈触发重载，读回相同内容后自然吸收，无需跳过标志，0.3s 防抖；外部修改靠监听同步进内存，intent 方法（含 `updatePaletteFrame`）不读盘，内存即真相；`reloadSilent` 读盘+解码在后台线程，主线程只应用结果，token 守卫防乱序覆盖；`AppSettingsManager` 可注入文件路径测试（`init(filePath:enableFileMonitoring:notifyLoadError:)`，测试关通知防泄漏系统弹窗），`shared` 走 `AppPaths`
